...
class ToolboxMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        card6 = ToolCard(
            "代码差异对比器",
            "逐行对比两个文件差异，新增/删除/修改着色，左右分栏同步滚动",
            "\U0001F4DD",
            self._open_diff,
            color=COLORS["hl"],
        )
    ...
    def _open_diff(self):
        """打开代码差异对比器"""
        if "diff" not in self._sub_windows:
            from ui.diff_window import DiffWindow
            self._sub_windows["diff"] = DiffWindow(on_back=self._show_toolbox)
        self.hide()
        self._sub_windows["diff"].show()
        self._sub_windows["diff"].raise_()
        self._sub_windows["diff"].activateWindow()
        logger.info("打开代码差异对比器")
    ...
    def _init_tray(self):
        ...
        act_show.triggered.connect(self._show_toolbox)
