...
class DynamicTracerSignals(QObject):
    ...
    ai_done = Signal(str)
    ai_error = Signal(str)
    interpreters_ready = Signal(list)  # 解释器检测完成，传递 [(priority, path, label), ...]
...
class DynamicTracerWindow(QWidget):
    ...
    def __init__(self, on_back=None):
        ...
        self.sig.interpreters_ready.connect(self._on_interpreters_ready)

        self._build_ui()
        self._on_status("请选择一个包含 Python 源码的文件夹", COLORS["warn"])
        apply_text_selectable(self)
        threading.Thread(target=self._do_detect_interpreters, daemon=True).start()
    ...
    def _build_input_area(self):
        ...
        self._interp_combo.setPlaceholderText("正在检测 Python 解释器...")
        self._interp_combo.addItem("正在检测 Python 解释器...", "")
        self._interp_combo.setEnabled(False)
    ...
    def _do_detect_interpreters(self):
        """后台线程：检测 Python 解释器"""
        try:
            candidates = self._detect_interpreters()
            self.sig.interpreters_ready.emit(candidates)
        except Exception as e:
            logger.error("解释器检测失败: %s", e)
            self.sig.interpreters_ready.emit([])

    def _do_detect_interpreters_with_folder(self, folder):
        """后台线程：检测指定文件夹附近的 Python 解释器"""
        try:
            candidates = self._detect_interpreters(folder)
            self.sig.interpreters_ready.emit(candidates)
        except Exception as e:
            logger.error("解释器检测失败: %s", e)
            self.sig.interpreters_ready.emit([])

    def _on_interpreters_ready(self, candidates):
        """主线程：解释器检测完成，更新下拉框"""
        self._interp_combo.clear()
        self._interp_combo.setEnabled(True)
        ...
        if candidates:
            ...
            self._interp_combo.addItem("未检测到 Python 解释器", "")
    ...
    def _detect_interpreters(self, folder_path: str = ""):
        ...
        # PyInstaller --onefile 模式下 sys.executable 是 EXE 本身，不是 Python 解释器
        # 执行它会重新启动整个程序，导致重复创建窗口和托盘图标
    ...
    def _on_browse(self):
        ...
        if folder:
            ...
            # Re-detect interpreters with folder context (in background thread)
            threading.Thread(target=self._do_detect_interpreters_with_folder, args=(folder,), daemon=True).start()
    ...
    def _run_tracer_subprocess(self, entry_path: str, var_name: str, run_args: list, target_lines: list = None):
        ...
        # 计算项目根目录（包含 core/ 的目录）
        # PyInstaller onefile 模式下，资源文件在 sys._MEIPASS 下
